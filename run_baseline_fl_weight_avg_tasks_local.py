...
def run_task(args):
    dataset, lr, bert_model, batchsize, warmup, gpuid, bert_dropout, seed, attention = args
    home_path = os.getcwd()
    taskname = dataset
    ...
    cmd = "python train_baseline_fl_weights.py --seed {seed} --dataset {dataset_list} --data_dir {data_dir} " \
          ...
          "--attention {attention} --sample_step 200 ".format(
        ...
        gpuid=gpuid,
        gpus=",".join([str(i) for i in range(len(gpuid.split(",")))]),
        attention=attention
    )
    cmd = "CUDA_VISIBLE_DEVICES={gpuid} {cmd} >{logfile} 2>&1".format(
        taskname=taskname,
        cmd=cmd,
        logfile=logfile,
        gpuid=gpuid
...
def main():
    ...
    parser.add_argument('--attentions', default="avg,loss", type=str)
            ...
            for batchsize in batchsizes:
                for attention in attentions:
                    task_list.append([args.datasets, lr, args.bert_model, batchsize, warmup, args.gpu,
                                  args.bert_dropout, args.seed, attention])

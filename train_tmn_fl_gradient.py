...
class TerminalInstructor():
    def __init__(self, args, dataset, device):
        ...
        self.data_processor = DataProcessor(args.data_dir, self.tokenizer,
                                            w_vocab_path=args.w_vocab_path,
    ...
    def train(self, model, step=1, coe_grad=1.0):
    ...
    def run(self):
        ...
        for epoch in trange(args.num_epoch, desc="Epoch"):
            ...
            for _ in range(steps_per_epoch):
                loss_list = []
                for d_i, dataset in enumerate(dataset_list):
                    terminal = terminal_dict[dataset]
                    _n_correct, _n_total, _loss_total = terminal.train(model, step=args.sample_step, coe_grad=att_list[d_i])
                    n_correct += _n_correct
                    n_total += _n_total
                    loss_total += _loss_total
                    loss_list.append(_loss_total)
                if args.attention == "loss":
                    loss_list = [(abs(l)+0.0001)**0.5 for l in loss_list]
                    sum_loss = sum(loss_list)
                    att_list = [l/sum_loss for l in loss_list]

...
def run_task(args):
    ...
    output_dir = "{home_path}/results/absa_fl_baseline_single_domain".format(
        home_path=home_path
    )

    cmd = "python train_baseline.py --seed {seed} --dataset {dataset} --test_dataset {dataset} --data_dir {data_dir} " \
    ...
    cmd = "srun --job-name ft_g_sng --mem=80GB --cpus-per-task=4 -n 1 --gres=gpu:1 {cmd} >{logfile} 2>&1".format(
        taskname=taskname,
        cmd=cmd,
        logfile=logfile,
        gpu=1
...
def main():
    ...
    lrs = [1e-5, 3e-5, 5e-5]
    warmups = [0.06]
    for dataset in ["laptop", "rest14", "rest15", "rest16", "twitter"]:
        for lr in lrs:
            for warmup in warmups:
                for batchsize in batchsizes:
                    task_list.append([dataset, lr, args.bert_model, batchsize, warmup, args.gpu,

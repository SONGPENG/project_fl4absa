...
def run_task(args):
    ...
    output_dir = os.path.join(home_path,
                "results/absa_FL_tmn_grad_topic_{topic_method}".format(
                attention=attention, topic_method=topic_method))

    dataset_list = "laptop,rest14,twitter"

    cmd = "python -m torch.distributed.launch --nproc_per_node=1 train_FL_tmn.py --seed {seed} --dataset {dataset_list} --data_dir {data_dir} " \
          "--bert_model {bert_model}  --batch_size {batchsize} --bert_dropout {bert_dropout} --outdir {output_dir} " \
          "--num_epoch 30 --warmup_proportion {warmup} --learning_rate {lr} " \
          "--topic_method {topic_method} --attention {attention} --fp16 " \
    ...
    cmd = "CUDA_VISIBLE_DEVICES=1 {cmd} >{logfile} 2>&1".format(
        cmd=cmd,
        logfile=logfile
    )
    print(cmd)

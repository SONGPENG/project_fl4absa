...
class Instructor:
    ...
    def _train(self, model, optimizer, scheduler, train_data_loader_dict, dev_dataloader_dict, test_dataloader_dict):
        path = None
    ...
    def run(self):
        ...
        x = 0
        for dataset in self.dataset_list:
            train_dataloader_dict[dataset] = self.data_processor.get_train_dataloader(dataset)
            x += len(train_dataloader_dict[dataset])
        ...
        num_train_optimization_steps = int(
            x / self.args.gradient_accumulation_steps) * self.args.num_epoch

        print("trainset: {}, batch_size: {}, gradient_accumulation_steps: {}, num_epoch: {}, num_train_optimization_steps: {}".format(
            x * self.args.batch_size, self.args.batch_size, self.args.gradient_accumulation_steps,
        ...
        self._train(model, optimizer, scheduler, train_dataloader_dict, dev_dataloader_dict, test_dataloader_dict)
...
def main():
    ...
    if args.local_rank == -1:
        ...
        n_gpu = 1
        sharefile = os.path.join(args.outdir, "{}.sharefile".format('tmn'))
        args.init_method = "file://{}".format(sharefile)

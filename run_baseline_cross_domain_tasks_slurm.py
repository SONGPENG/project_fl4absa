...
def run_task(args):
    ...
    output_dir = "{home_path}/results/absa_fl_baseline_cross_domain".format(
        home_path=home_path
    )

    dataset_list = "laptop,rest14,twitter"

    cmd = "python train_baseline.py --seed {seed} --dataset {dataset_list} --data_dir {data_dir} " \
    ...
    cmd = "srun --job-name ft_g_fl_crs --ntasks=1 --ntasks-per-node=1 --gres=gpu:1 {cmd} >{logfile} 2>&1".format(
        taskname=taskname,
        cmd=cmd,
        logfile=logfile,
        gpu=1
